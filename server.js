...
const corsOptions = {
  origin: [
    "https://playground-050-frontend.vercel.app",
    "http://localhost:5173",
  ],
  credentials: true,
...
app.get("/", (req, res) => {
  res.json({ message: "Chars Database! Browse your favorite characters" });
});
...
app.post("/chars", async (req, res) => {
  const { charName, charSeries, charAuth } = req.body;

  if (!charAuth || !charName || !charSeries) {
    return res
      .status(400)
  ...
  try {
    const newChar = new Char({ charAuth, charName, charSeries });
...
app.put("/chars/:id", async (req, res) => {
  try {
    const editedChar = await Char.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    });

    if (!editedChar) {
      return res.status(404).json({ message: "Unable to find the char" });
    }
...
app.delete("/chars/:id", async (req, res) => {
  try {
    const deletedChar = await Char.findByIdAndDelete(req.params.id);

    if (!deletedChar) {
      return res.status(404).json({ message: "Unable to find the char" });
    }

    res.json({ message: "Char deleted successfully", char: deletedChar });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Internal server error", error: error.message });
